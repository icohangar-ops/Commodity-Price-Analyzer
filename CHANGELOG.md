...
- `calculate_black_mass_payables` — 80% grade multiplier on LME 3-month Ni/Co; counterparties: Counterparty A, Counterparty B, Counterparty C
- `calculate_primary_offtaker_mhp_offtake` — 5% floor discount; 10% profit share above $18,000/mt Ni threshold; Fastmarkets MB CO-0005 basis
- `calculate_lithium_carbonate_gtc` — $18,000/mt floor, $25,000/mt ceiling; Fastmarkets Li₂CO₃ 99.5% CIF basis
- `calculate_composite_feedstock` — 90% Li @ 70% payable, 3% Ni @ 90%, 2% Co @ 90%; mixed Fastmarkets/LME composite basis
...
**Memory**
- `OpCo Contract Parameters` (load) — shared, persistent contract configuration
- `Historical Pricing Data` (store, append) — shared, persistent audit log

**Prompts**
- Stage 1 system prompt: OpCo Commodity Price Analyzer (consolidated from multiple prompt segments)
- Stage 2 system prompt: Financial Analyst specialized in OpCo contract economics
