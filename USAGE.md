...
```
**Triggers:** AlphaVantage NICKEL + COBALT fetch → `calculate_black_mass_payables(grade_multiplier=0.80)` → narrative with USD/mt payable values, grade-adjusted breakdown, and comparison to 100% theoretical.

```
What's our Ni/Co margin vs Offtaker this month on Counterparty A output?
```
**Triggers:** Same calculation, with the Financial Analyst model framing the answer specifically around the Counterparty A counterparty context loaded from OpCo Contract Parameters memory.
...
```
**Triggers:** AlphaVantage NICKEL fetch → `calculate_primary_offtaker_mhp_offtake()` → checks `profit_share_triggered` boolean → narrative explains whether Ni is above/below $18,000/mt threshold and, if triggered, the exact dollar amount flowing to MHP as profit share.

**Example response (when triggered):**
> *LME Nickel is currently at $21,400/mt, which is $1,400/mt above the $18,000/mt profit-share threshold. Under the Primary Offtaker agreement, 15% of this incremental value — $210/mt — flows back to MHP. The realized Ni payable after the 5% floor discount and profit share is $19,488/mt, compared to a theoretical maximum (at 100% spot) of $21,400/mt. Payable capture is 91.1%.*

```
What is the impact of the 5% floor discount on our MHP realized value this month?
```
...
```
**Triggers:** AlphaVantage Li price fetch → `calculate_lithium_carbonate_gtc(floor=18000, ceiling=25000)` → `floor_protection_active` and `ceiling_cap_active` booleans → narrative states which condition applies and the realized vs spot difference.

**Example response (floor active):**
> *Fastmarkets Li₂CO₃ 99.5% CIF is currently at $17,800/mt, which is below the GTC floor of $18,000/mt. The floor protection is active — OpCo realizes $18,000/mt rather than spot, a benefit of $2,200/mt. At current volumes this represents a meaningful positive variance vs unprotected spot exposure.*
...
```
What's the all-in realized value per tonne of Counterparty B feedstock at today's prices?
```
**Triggers:** AlphaVantage NICKEL + COBALT + Li fetch → `calculate_composite_feedstock(feedstock_grade=0.90)` → breakdown of Li, Ni, and Co contributions to total feedstock value.

```
How much of the Counterparty B feedstock value comes from Li vs the Ni/Co credits?
```
...
|---|---|
| Black Mass | −$850/mt (80% grade multiplier) |
| Primary Offtaker Floor | −$920/mt (90% = 1 − 5% discount) |
| Counterparty B Ni credit | −$27/mt (3% content × 90% payable) |
| Li Carbonate GTC | No impact (Li-indexed only) |

```
What's our total P&L sensitivity to a 10% move in Cobalt?
```
**Triggers:** AlphaVantage COBALT fetch → sensitivity re-run with Co ±10% shock → aggregate dollar impact across black mass, Primary Offtaker, and Counterparty B contracts.
...
```
**Triggers:** Regulations.Gov search for "IRA lithium carbonate battery" → AI Model synthesizes relevant regulatory documents → Financial Analyst frames implications for OpCo's GTC structure.
...
- **Make trading recommendations** — the agent provides commercial intelligence; investment or hedging decisions remain with OpCo's commercial team.
