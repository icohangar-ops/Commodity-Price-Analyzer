...
This document covers the two MCP tools registered in the Airia flow, their exact API configurations, how symbols map to OpCo contract indices, and the normalization rules the AI Model (Stage 1) must apply before passing prices to the Python step.
...
### Index Mapping — AlphaVantage to OpCo Contract Reference

| OpCo Contract Reference | AlphaVantage Function | Notes |
|---|---|---|
...
**Index cross-reference:** In some queries the model uses Regulations.Gov to cross-check whether an index referenced in a OpCo contract (e.g. Fastmarkets MB CO-0005) has been cited in a regulatory filing, which helps verify that the correct index definition is being used.
...
| "Counterparty A output" | AlphaVantage NICKEL + COBALT | `black_mass` (Counterparty A counterparty) |
