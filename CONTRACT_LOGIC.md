...
> **Note on contract parameters:** All commercial terms in this repository
> (grade multipliers, discounts, profit-share thresholds, floors/ceilings,
> counterparty labels) are **illustrative examples only**. Real terms are
> confidential and must be supplied via private configuration at runtime.
...
**Commercial context:** Black mass is the intermediate output from battery shredding. Payables reference the LME 3-month average for Ni and Co with an 80% grade multiplier applied to contained metal value. Active counterparties: Counterparty A, Counterparty B, Counterparty C.

```python
def calculate_black_mass_payables(ni_price, co_price, grade_multiplier=0.80):
    """Black mass payables based on Ni/Co prices and grade multiples.
    Basis: LME 3-month average.
    Grade multiplier default: 80% of contained metal value (mixed black mass).
    """
...
- `grade_multiplier` — recorded for audit trail (default 0.80)
...
- **5% floor discount** — Primary Offtaker receives Ni and Co at 5% below spot as a structural discount
- **10% profit share above $18,000/mt Ni** — when LME Ni exceeds this threshold, 10% of the incremental value above threshold flows back to MHP as a profit share

```python
THRESHOLD_PRICE_PER_MT = 18000  # USD/mt — Ni profit-share trigger
FLOOR_DISCOUNT = 0.05           # 5% floor discount applied to both Ni and Co
PROFIT_SHARE = 0.10             # 15% of incremental Ni value above threshold
...
def calculate_primary_offtaker_mhp_offtake(
    ...
    Structure:
    - Primary Offtaker receives 5% discount to spot on both Ni and Co (floor discount).
    - For Ni prices above $18,000/mt, Primary Offtaker shares 10% of the incremental
      value above $18,000/mt back with MHP as a profit share.
    """
    # Base floor prices after 5% discount
...
If Ni = $22,000/mt:
- Floor Ni = $22,000 × 0.90 = $20,240/mt
- Incremental above threshold = $22,000 − $18,000 = $2,000
...
**Commercial context:** Lithium Carbonate is sold under a General Trading Confirmation (GTC) structure referencing Fastmarkets Li₂CO₃ 99.5% CIF. The agreement includes a hard floor ($18,000/mt) and ceiling ($25,000/mt) that cap the effective price regardless of spot movement.

```python
def calculate_lithium_carbonate_gtc(
    li_price,
    contract_floor=18000,
    contract_ceiling=25000
):
...
**State logic:**
- `floor_protection_active = True` → spot is below $18,000/mt; OpCo is protected and receives the floor
- `ceiling_cap_active = True` → spot is above $25,000/mt; OpCo's upside is capped
- Neither active → effective price equals spot (within the band)

---

### 4. Counterparty B Feedstock

**Commercial context:** Counterparty B purchases battery recycling feedstock from OpCo. The agreement is a multi-commodity payable structure: the primary value driver is lithium content (90% grade, 70% payable), with credit for residual Ni (3% content, 90% payable) and Co (2% content, 90% payable).

```python
def calculate_composite_feedstock(
    li_price,
    ni_price,
    co_price,
    feedstock_grade=0.90
):
    """Counterparty B feedstock pricing — multi-commodity payable structure.
    
    Li content:  90% of feedstock @ 70% payable
    Ni content:   3% of feedstock @ 90% payable
    Co content:   2% of feedstock @ 90% payable
    Basis: Mixed Fastmarkets/LME composite
    """
    li_payable = li_price * feedstock_grade * 0.70   # primary value driver
...
primary_offtaker = results["contract_calculations"]["primary_offtaker_mhp_offtake"]
if primary_offtaker["profit_share_triggered"]:
    results["business_insights"].append(
        ...
        f"the $18,000/mt threshold. An additional "
        f"${primary_offtaker['profit_share_amount']}/mt is shared back with MHP."
    )

# Black mass summary
black_mass = results["contract_calculations"]["black_mass_payables"]
results["business_insights"].append(
...
li_gtc = results["contract_calculations"]["lithium_carbonate_gtc"]
...
results["sensitivity_analysis"] = {
    "ni_plus_1000": {
        "black_mass_delta":             round(1000 * 0.80, 2),   # = $850/mt
        "primary_offtaker_floor_delta": round(1000 * 0.90, 2),  # = $920/mt
    },
    "co_plus_1000": {
        "black_mass_delta":             round(1000 * 0.80, 2),   # = $850/mt
        "primary_offtaker_floor_delta": round(1000 * 0.90, 2),  # = $920/mt
    ...
    "li_plus_1000": {
        ...
        "feedstock_delta": round(1000 * 0.90 * 0.70, 2),        # = $690/mt
    },
}
```

**Interpreting the Ni sensitivity:**
A $1,000/mt increase in LME Ni adds $850/mt to black mass payables (80% multiplier) and $920/mt to the Primary Offtaker floor price (90% = 1 − 5% discount). If that move also pushes Ni above the $18,000/mt profit-share threshold, an additional 15% of the increment flows to MHP — which the `calculate_primary_offtaker_mhp_offtake` function captures automatically.
...
|---|---|---|
| Black Mass | Grade multiplier | 80% of LME 3-month Ni/Co |
| Black Mass | Counterparties | Counterparty A, Counterparty B, Counterparty C |
| Primary Offtaker MHP | Floor discount | 5% below Fastmarkets MB |
| Primary Offtaker MHP | Profit share threshold | $18,000/mt Ni |
...
| Li Carbonate GTC | Floor | $18,000/mt |
| Li Carbonate GTC | Ceiling | $25,000/mt |
| Li Carbonate GTC | Basis | Fastmarkets Li₂CO₃ 99.5% CIF |
| Counterparty B Feedstock | Li content / payable | 90% grade · 70% payable |
| Counterparty B Feedstock | Ni content / payable | 3% content · 90% payable |
| Counterparty B Feedstock | Co content / payable | 2% content · 90% payable |
...
results["contract_calculations"]["black_mass_payables"]        = calculate_black_mass_payables(ni_price, co_price)
results["contract_calculations"]["primary_offtaker_mhp_offtake"] = calculate_primary_offtaker_mhp_offtake(ni_price, co_price)
results["contract_calculations"]["lithium_carbonate_gtc"]       = calculate_lithium_carbonate_gtc(li_price)
results["contract_calculations"]["composite_feedstock"]          = calculate_composite_feedstock(li_price, ni_price, co_price)
