...
| Asset | Rule |
|---|---|
| **Email** | Dedicated Gmail (e.g., `agent.mailbox@gmail.com`). Never your personal or corporate email. |
...
**Why this matters beyond security:** Context separation prevents the agent's web scraping from polluting your personal accounts, triggering bot-detection bans on your main profiles, or creating attribution problems with your corporate identity. If the agent's credentials are compromised, the blast radius is limited to the burner identity — not your the operator systems.
...
1. **Never route the operator data** (investor materials, customer names, contract terms, financial models) through any external API — Chinese or otherwise.
...
```
"Current Status: Executing Step 3 of 5. Confidence: 80%. No blockers."
```
