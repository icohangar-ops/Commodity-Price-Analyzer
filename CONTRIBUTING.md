...
```python
results["contract_calculations"]["new_contract"] = calculate_new_contract(
...
```python
new_calc = results["contract_calculations"]["new_contract"]
...
### 4. Update the OpCo Contract Parameters memory

Add the new contract's parameters to the `OpCo Contract Parameters` memory so the AI Model (Stage 1) knows it exists and can select it when relevant queries arrive.
...
Add a row to the symbol map table showing how the new source's field names map to OpCo contract index references.
...
3. Add the corresponding contract functions for any OpCo contracts that reference Mn or Cu
...
For parameter changes that don't require new logic (e.g., the profit-share threshold changes from $18,000/mt to $22,000/mt):

1. Update the constant in the Python Code step:
   ```python
   THRESHOLD_PRICE_PER_MT = 22000  # updated from 18000
   ```
2. Update the OpCo Contract Parameters memory with the new value
