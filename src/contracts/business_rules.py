"""
Contract Business Rules Engine.

Calculates payables, floor/ceiling logic, profit shares, and sensitivity
analysis for commodity offtake contracts.

All contract parameters in this module are ILLUSTRATIVE EXAMPLES.
Real commercial terms are confidential and must be supplied via private
configuration (function arguments / environment), never committed here.
"""
import logging
...
# =============================================================================
# Illustrative default contract parameters
# =============================================================================
THRESHOLD_PRICE_PER_MT = 18000   # Ni threshold for profit share (USD/mt)
FLOOR_DISCOUNT = 0.05            # 5% floor discount
PROFIT_SHARE = 0.10              # 10% profit share above threshold
GRADE_MULTIPLIER = 0.80          # Black mass grade multiplier
LITHIUM_FLOOR = 18000            # Lithium carbonate contract floor (USD/mt)
LITHIUM_CEILING = 25000          # Lithium carbonate contract ceiling (USD/mt)
FEEDSTOCK_GRADE = 0.90           # Composite feedstock grade factor
FEEDSTOCK_LI_FACTOR = 0.70       # Lithium payable factor for feedstock
...
def calculate_black_mass_payables(
    ...
    Args:
        ...
        grade_multiplier: Grade adjustment factor (illustrative default 80%).
...
def calculate_primary_offtaker_mhp_offtake(
    ...
    """
    Primary Offtaker MHP offtake pricing with floor discounts and profit
    sharing.

    Args:
        ni_price: Nickel price in USD/mt.
        co_price: Cobalt price in USD/mt.
        floor_discount: Floor discount percentage (illustrative default 5%).
        profit_share_threshold: Ni price threshold for profit share
            (illustrative default $18k/mt).
        profit_share: Share of the incremental price above the threshold
            returned to the offtaker (illustrative default 10%).

    Returns:
        Dict with MHP offtake calculations including profit share trigger
        status.
    """
    ...
    return {
        ...
        "basis": "Reference index monthly average",
...
def calculate_lithium_carbonate_gtc(
    li_price: float,
    contract_floor: float = LITHIUM_FLOOR,
    contract_ceiling: float = LITHIUM_CEILING,
) -> dict:
    """
    ...
    Args:
        ...
        contract_floor: Contract floor price (illustrative default $18,000/mt).
        contract_ceiling: Contract ceiling price (illustrative default $25,000/mt).
    ...
    return {
        ...
        "basis": "Reference lithium carbonate 99.5% CIF index",
    }
...
def calculate_composite_feedstock(
    ...
    ni_price: float,
    co_price: float,
    feedstock_grade: float = FEEDSTOCK_GRADE,
) -> dict:
    """
    Composite feedstock pricing based on contained value.
    ...
    Args:
        ...
        ni_price: Nickel price in USD/mt.
        co_price: Cobalt price in USD/mt.
        feedstock_grade: Feedstock grade factor (illustrative default 90%).

    Returns:
        Dict with feedstock payable calculations.
    """
    li_payable = li_price * feedstock_grade * FEEDSTOCK_LI_FACTOR
    ...
    return {
        ...
        "basis": "Composite reference index",
    }
...
def run_all_calculations(prices: dict) -> dict:
    """
    Execute all contract calculations.
    ...
    calcs = results["contract_calculations"]
    calcs["black_mass_payables"] = calculate_black_mass_payables(ni_price, co_price)
    calcs["primary_offtaker_mhp_offtake"] = calculate_primary_offtaker_mhp_offtake(ni_price, co_price)
    calcs["lithium_carbonate_gtc"] = calculate_lithium_carbonate_gtc(li_price)
    calcs["composite_feedstock"] = calculate_composite_feedstock(li_price, ni_price, co_price)
    ...
    primary_offtaker = calcs["primary_offtaker_mhp_offtake"]
    if primary_offtaker["profit_share_triggered"]:
        results["business_insights"].append(
            ...
            f"${THRESHOLD_PRICE_PER_MT:,}/mt threshold. Additional "
            f"${primary_offtaker['profit_share_amount']}/mt shared back with MHP as profit share."
        )

    black_mass = calcs["black_mass_payables"]
    results["business_insights"].append(
    ...
    li_gtc = calcs["lithium_carbonate_gtc"]
    ...
    results["sensitivity_analysis"] = {
        ...
        "li_plus_1000": {
            ...
            "feedstock_delta": round(1000 * FEEDSTOCK_GRADE * FEEDSTOCK_LI_FACTOR, 2),
        },
    }

    logger.info(
        "calculations_complete ni=%s co=%s li=%s profit_share=%s",
        ni_price,
        co_price,
        li_price,
        primary_offtaker["profit_share_triggered"],
    )
