...
try:  # Optional dependency; fallback prices work without it.
    import requests
...
class DataFetcher:
    ...
    def __init__(self):
        ...
        logger.info("data_fetcher_initialized has_av_key=%s", bool(self.alphavantage_key))
    ...
    def fetch_metal_price(self, metal: str) -> Optional[dict]:
        ...
        """
        if not self.alphavantage_key or requests is None:
            logger.warning("alphavantage_unavailable metal=%s", metal)
            return self._fallback_price(metal)

        function = self.METAL_FUNCTIONS.get(metal.lower(), metal.upper())
        ...
        try:
            ...
            if price_data:
                logger.info(
                    "price_fetched metal=%s price=%s source=alphavantage",
                    metal,
                    price_data.get("price"),
                )
                price_data["source"] = "AlphaVantage"
                price_data["fetched_at"] = datetime.now(timezone.utc).isoformat()

            return price_data

        except requests.exceptions.RequestException as e:
            logger.error("price_fetch_failed metal=%s error=%s", metal, e)
            return self._fallback_price(metal)
    ...
    def _parse_alphavantage_response(self, data: dict, metal: str) -> Optional[dict]:
        ...
        if "Error Message" in data or "Note" in data:
            logger.warning("alphavantage_error response=%s", data)
            return None

        logger.warning("unknown_response_format keys=%s", list(data.keys()))
        return None
    ...
    def _fallback_price(self, metal: str) -> dict:
        ...
        logger.warning("using_fallback_price metal=%s price=%s", metal, price)
        return {
            "metal": metal,
            "price": price,
    ...
    def search_regulations(self, search_term: str, page_size: int = 10) -> Optional[dict]:
        ...
        """
        if not self.regulations_key or requests is None:
            logger.warning("regulations_search_unavailable")
            return None
        ...
        try:
            ...
            data = response.json()
            logger.info(
                "regulations_search_complete term=%s count=%s",
                search_term,
                len(data.get("data", [])),
            )
            return data
        except requests.exceptions.RequestException as e:
            logger.error("regulations_search_failed error=%s", e)
            return None
    ...
    def get_historical_prices(self, metal: str, days: int = 30) -> list:
        ...
        For now, returns the current price as a single-element history.
        ...
        """
        logger.info("historical_data_requested metal=%s days=%s", metal, days)
...
# Backwards-compatible alias for the pre-release class name.
CommodityDataFetcher = DataFetcher
